...
Matrix <T> Image::reverse(const Matrix <T>& mat) {
  ...
  for (std::size_t hIndex = 0, resHIndex = res.getWidth() - 1; hIndex < mat.getHeight(), resHIndex > 0; ++hIndex, --resHIndex) {
    for (std::size_t wIndex = 0, resWIndex = res.getHeight() - 1; wIndex < mat.getWidth(), resWIndex > 0; ++wIndex, --resWIndex) {
      const std::size_t matIndex = (hIndex * mat.getWidth() + wIndex) * mat.getChannelCount();
      const std::size_t resIndex = (resHIndex * res.getWidth() + resWIndex) * res.getChannelCount();
